...
# Jetson Orin Nano Deployment Script for Single Server
# This script builds and runs the single server container on Jetson

set -e

echo "=== Jetson Orin Nano Single Server Deployment ==="
...
if [ ! "$(ls -A models 2>/dev/null)" ]; then
    ...
    echo "  - models/ensemble-effnet-c5-lr-0.005-tin/encoder1.onnx"
    echo "  - models/ensemble-effnet-c5-lr-0.005-tin/classifier1.onnx"
    echo "  - models/ensemble-effnet-c5-lr-0.005-tin/single.onnx"
    echo "  - etc."
    echo ""
...
# Run the container
echo "Starting the single server container..."
