...
from ensemble_efficient_net_b0 import EnsembleEfficientNet, EfficientNetEarly
...
def main():
    ...
    if args.cut_point > 0:
        model = EfficientNetEarly(
            num_classes=args.num_classes, cut_point=args.cut_point)
    else:
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                ...
                mlflow.log_metric("Learning rate", lr, step=num_updates)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            acc = utils.accuracy(output, target, topk=(1,))[0]
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    f'Acc@1: {acc_m.val:>7.3f} ({acc_m.avg:>7.3f})  '
                )

    metrics = OrderedDict(
        [('loss', losses_m.avg), ('acc', acc_m.avg)])

    mlflow.log_metric("valid loss", losses_m.avg, step=epoch)
    mlflow.log_metric("Valid accuracy", acc_m.avg, step=epoch)
