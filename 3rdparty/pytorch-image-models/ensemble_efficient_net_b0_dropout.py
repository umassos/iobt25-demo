...
class EfficientNetHeadDropout(nn.Module):
    def __init__(self, num_classes: int, input_filters: int, drop_out_rate: float = 0.3, embedding_dropout: bool = 0.2):
        super(EfficientNetHeadDropout, self).__init__()
    ...
    def forward(self, inputs1, inputs2):
        ...
        # Dropout the embeddings with certain probability during training
        if self.training:
            rand = random.random()
            if rand <= self._embedding_dropout / 2:
                x1 = torch.zeros_like(x1)
            elif self._embedding_dropout / 2 < rand <= self._embedding_dropout:
...
class EnsembleEfficientNet(nn.Module):
    def __init__(self, num_classes: int, cut_point: int, width_ratio: float = 1.0, num_repeat: int = None,
                 last_channels: int = 1280, head_type: str = '', hidden_dim: int = 256):
        ...
        self.encoder1 = EfficientNetEmbeddingEarly(
            num_classes=num_classes, cut_point=cut_point, last_channels=last_channels)
        self.encoder2 = EfficientNetEmbeddingEarly(
            num_classes=num_classes, cut_point=cut_point, last_channels=last_channels)

        # if self.head_type == 'fc':
        #     self.classifier_comb = EfficientNetHeadFC(
        #         num_classes=self.num_classes, input_filters=self.encoder1.encoder.last_channels + self.encoder2.encoder.last_channels, hidden_dim=self.hidden_dim)
        # elif self.head_type == 'cnn':
        #     self.classifier_comb = EfficientNetHeadCNN(
        #         self.num_classes, self.encoder1.encoder.last_channels + self.encoder2.encoder.last_channels, out_channels=self.last_channels)
        # else:
        self.classifier_comb = EfficientNetHeadDropout(self.num_classes,
...
class EfficientNetEmbeddingEarly(nn.Module):
    def __init__(self, num_classes: int, cut_point: int, last_channels: int = 1280):
        super(EfficientNetEmbeddingEarly, self).__init__()
        self.num_classes = num_classes
        self._cut_point = cut_point
        self.last_channels = last_channels
