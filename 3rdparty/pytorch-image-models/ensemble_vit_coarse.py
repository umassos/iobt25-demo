...
class ViTEarly(nn.Module):
    def __init__(self, image_size, num_classes: int, num_coarse_classes: int, cut_point: int):
        ...
        self.classifier = nn.Linear(self.encoder._dim, num_coarse_classes)
...
class EnsembleViTCoarse(nn.Module):
    def __init__(self, image_size, num_classes: int, num_coarse_classes: int, cut_point: int):
        super(EnsembleViTCoarse, self).__init__()
        self._num_classes = num_classes
        self._cut_point = cut_point

        self.encoder1 = ViTEarly(
            image_size, num_classes, num_coarse_classes, cut_point)
        self.encoder2 = ViTEarly(
            image_size, num_classes, num_coarse_classes, cut_point)

        self.classifier_comb = ViTHead(
        ...
        print(
            f"Encoder-1 # params: {self.encoder1_stat.total_params}, last channels {self.encoder1.encoder._dim}")
        print(
            f"Encoder-2 # params: {self.encoder2_stat.total_params}, last channels {self.encoder2.encoder._dim}")
        ...
        # self.encoder1 = ViTEncoder(image_size, patch_size, num_classes, dim, depth, heads,
        #                            mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.)

        # self.encoder2 = ViTEncoder(image_size, patch_size, num_classes, dim, depth, heads,
        #    mlp_dim, pool='cls', channels=3, dim_head=64, dropout=0., emb_dropout=0.)
