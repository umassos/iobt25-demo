...
class EnsembleLoss(object):
    ...
    def __call__(self, output, target, output_lengths, target_lengths):
        # y1_loss = self.loss_fn(
        #     output[0], target, output_lengths, target_lengths)
        # y2_loss = self.loss_fn(
        #     output[1], target, output_lengths, target_lengths)
        y_comb_loss = self.loss_fn(
            output[2], target, output_lengths, target_lengths)
        return y_comb_loss
...
class EnsembleContrastiveLoss(object):
    def __init__(self, ensemble_loss_fn, eps=4, lamb=1e-2, use_contrastive=True):
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target, target_lengths) in enumerate(loader):
            ...
            wer_comb = word_error_rate(output[2], target, target_lengths)
