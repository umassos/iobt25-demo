...
from ensemble_nano_gpt import GPTConfig, GPT, EnsembleGPT
...
group.add_argument('--lr-decay-iters', type=int, default=200000, metavar='N',
                   help='iterations over which to decay LR (default: 200000)')
...
group.add_argument('--checkpoint-nn1', default='', type=str, metavar='PATH',
                   help='Initialize NN-1 Backbone model from this checkpoint (default: none)')
group.add_argument('--checkpoint-nn2', default='', type=str, metavar='PATH',
...
# model init
model_args = dict(n_layer=args.cut_point, n_head=args.n_head, n_embd=args.n_embd, block_size=args.block_size,
...
if args.init_from == 'scratch':
    ...
    model = EnsembleGPT(gptconf)
elif args.init_from == 'ss1':
    print(f"Resuming training from {args.initial_checkpoint}")
    # resume training from a checkpoint.
    if meta_vocab_size is None:
        model_args['vocab_size'] = args.vocab_size
    ckpt1_path = os.path.join(args.checkpoint_nn1, 'ckpt.pt')
    ckpt2_path = os.path.join(args.checkpoint_nn2, 'ckpt.pt')
    checkpoint1 = torch.load(ckpt1_path, map_location=device)
    checkpoint2 = torch.load(ckpt2_path, map_location=device)

    checkpoint1_model_args = checkpoint1['model_args']
    checkpoint2_model_args = checkpoint2['model_args']
    ...
    for k, v in list(state_dict2.items()):
        if k.startswith(unwanted_prefix):
            state_dict2[k[len(unwanted_prefix):]] = state_dict2.pop(k)
    model.encoder1.load_state_dict(state_dict1)
    model.encoder2.load_state_dict(state_dict2)
    model.freeze_and_unfreeze_encoders(freeze_nn1=True, freeze_nn2=True)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        losses_comb = AverageMeter()
        total_loss = AverageMeter()
        log_name = 'Loss ' + split
        for k in range(args.eval_iters):
            X, Y = get_batch(split)
            with ctx:
                _, (loss1, loss2, loss_comb) = model(X, Y)
            losses_1.update(loss1.item())
            losses_2.update(loss2.item())
            losses_comb.update(loss_comb.item())
            total_loss.update(
                args.loss_weights[0]*loss1.item() + args.loss_weights[1]*loss2.item() + args.loss_weights[2]*loss_comb.item())
            if k % args.log_interval == 0:
                _logger.info(
                    f'{log_name}: [{k:>4d}/{args.eval_iters}]  '
                    f'Total Loss: {total_loss.val:>7.3f} ({total_loss.avg:>7.3f})  '
                    f'Loss@1: {losses_1.val:>7.3f} ({losses_1.avg:>7.3f})  '
                    f'Loss@2: {losses_2.val:>7.3f} ({losses_2.avg:>7.3f}) '
                    f'Loss@comb: {losses_comb.val:>7.3f} ({losses_comb.avg:>7.3f})'
                )
        out[split] = {'loss1': losses_1.avg,
                      'loss2': losses_1.avg,
                      'loss_comb': losses_comb.avg,
                      'total_loss': total_loss.avg}

    mlflow.log_metric("Val total loss",
                      total_loss.avg, step=iter_num)
    mlflow.log_metric("Val loss_1", losses_1.avg, step=iter_num)
    mlflow.log_metric("Val loss_2", losses_2.avg, step=iter_num)
    mlflow.log_metric("Val loss_comb",
                      losses_comb.avg, step=iter_num)
    model.train()
    return out
...
with mlflow.start_run(run_name=args.run_name):
    try:
        ...
        mlflow.log_param("NN-1 encoder number of parameters",
                         original_model.encoder1_stat.total_params)
        mlflow.log_param("NN-1 classifier number of parameters",
                         original_model.classifier1_stat.total_params)
        mlflow.log_param("NN-2 encoder number of parameters",
                         original_model.encoder2_stat.total_params)
        mlflow.log_param("NN-2 classifier number of parameters",
                         original_model.classifier2_stat.total_params)
        mlflow.log_param("NN-12 number of parameters",
                         original_model.head_stat.total_params)
        ...
        while True:
            ...
            if iter_num % args.eval_interval == 0 and master_process:
                ...
                if losses['val']['loss_comb'] < best_val_loss or args.always_save_checkpoint:
            ...
            for micro_step in range(args.gradient_accumulation_steps):
                ...
                with ctx:
                    logits, all_loss = model(X, Y)
                    # Compute the combined loss
                    loss = args.loss_weights[0] * all_loss[0] + args.loss_weights[1] * \
                        all_loss[1] + args.loss_weights[2] * all_loss[2]
                    # scale the loss to account for gradient accumulation
                    losses_1.update(all_loss[0].item())
                    losses_2.update(all_loss[1].item())
                    losses_comb.update(all_loss[2].item())
            ...
            mlflow.log_metric("Train total loss",
                              total_loss.avg, step=iter_num)
            mlflow.log_metric("Train loss_1", losses_1.avg, step=iter_num)
            mlflow.log_metric("Train loss_2", losses_2.avg, step=iter_num)
            mlflow.log_metric("Train loss_comb",
                              losses_comb.avg, step=iter_num)
