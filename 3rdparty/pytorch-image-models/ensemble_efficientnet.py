...
EFFICIENTNET_B1_BLOCK_ARGS = [
    BlockArgs(num_repeat=2, kernel_size=3, stride=1, expand_ratio=1, input_filters=32,
              output_filters=16, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=3, kernel_size=3, stride=2, expand_ratio=6, input_filters=16,
              output_filters=24, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=3, kernel_size=5, stride=2, expand_ratio=6, input_filters=24,
              output_filters=40, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=4, kernel_size=3, stride=2, expand_ratio=6, input_filters=40,
              output_filters=80, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=4, kernel_size=5, stride=1, expand_ratio=6, input_filters=80,
              output_filters=112, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=5, kernel_size=5, stride=2, expand_ratio=6, input_filters=112,
              output_filters=192, se_ratio=.25, id_skip=True),
    BlockArgs(num_repeat=2, kernel_size=3, stride=1, expand_ratio=6, input_filters=192,
              output_filters=320, se_ratio=.25, id_skip=True)
]
...
class EfficientNetHead(nn.Module):
    def __init__(self, num_classes: int, input_filters: int, out_channels: int = 1280, drop_out_rate: float = 0.3):
...
class EnsembleEfficientNet(nn.Module):
    def __init__(self, num_classes: int, cut_point: int, width_ratio: float = 1.0, num_repeat: int = None,
        ...
        # self.encoder1 = get_multiexit_efficientnet_b0(self.num_classes, 0, self._cut_point, 1, self.width_ratio)
        # self.encoder2 = get_multiexit_efficientnet_b0(self.num_classes, 0, self._cut_point, 1, self.width_ratio)
        self.encoder1 = get_multiexit_efficientnet_b1(
            self.num_classes, 0, self._cut_point, 1, self.width_ratio)
        self.encoder2 = get_multiexit_efficientnet_b1(
            self.num_classes, 0, self._cut_point, 1, self.width_ratio)
        self.classifier1 = LinearClassifier(
            self.num_classes, self.encoder1.last_channels)
        self.classifier2 = LinearClassifier(
            self.num_classes, self.encoder2.last_channels)
        # self.encoder12 = get_multiexit_efficientnet_b0(self.num_classes, self._cut_point, self._cut_point+1, 2)
        # self.encoder12 = get_efficientnet_encoder12(self.num_classes, self._cut_point, self._cut_point + 1, 2,
        #                                             num_repeat=self.num_repeat)
        # self.classifier_comb = nn.Sequential(
        #     self.encoder12,
        #     LinearClassifier(self.num_classes, self.encoder12.last_channels)
        # )

        # Freezing the branches for finetuning the head
        ...
        self.classifier_comb = EfficientNetHead(
            self.num_classes, self.encoder1.last_channels*2, self.last_channels)

        self.encoder1_stat = summary(self.encoder1, verbose=0)
        self.encoder2_stat = summary(self.encoder2, verbose=0)
        self.classifier1_stat = summary(self.classifier1, verbose=0)
        self.classifier2_stat = summary(self.classifier2, verbose=0)
        self.classifier_comb_stat = summary(self.classifier_comb, verbose=0)

        print("Ensemble EfficientNet B0 created")
        print(
            f"Encoder-1 # params: {self.encoder1_stat.total_params}, last channels {self.encoder1.last_channels}")
        print(
            f"Encoder-2 # params: {self.encoder2_stat.total_params}, last channels {self.encoder2.last_channels}")
...
def get_efficientnet_b1_block_args(width_ratio: float = 1.0):
    block_args = EFFICIENTNET_B1_BLOCK_ARGS.copy()
    for i in range(len(block_args)):
        if i != 0:
            block_args[i] = block_args[i]._replace(input_filters=int(block_args[i].input_filters * width_ratio),
                                                   output_filters=int(block_args[i].output_filters * width_ratio))
...
def get_multiexit_efficientnet_b1(num_classes: int, start_block: int, end_block: int,
                                  num_inputs: int, width_ratio: float = 1.0) -> AnytimeEfficientNetDepth:
    """ Get anytime efficientnet-b0 model """
    global_params = get_global_param("efficientnet-b1")
    global_params = global_params._replace(num_classes=num_classes)
    block_args = get_efficientnet_b1_block_args(width_ratio)
    model = AnytimeEfficientNetDepth(block_args, global_params, start_block=start_block,
                                     end_block=end_block, num_inputs=num_inputs)
