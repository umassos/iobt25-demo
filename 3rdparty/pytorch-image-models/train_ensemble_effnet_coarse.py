...
from ensemble_efficientnet_coarse import EnsembleEfficientNetCoarse
from ensemble_efficient_net_b1_coarse import EnsembleEfficientNetB1Coarse
from ensemble_resnet_50_coarse import EnsembleResnet50Coarse
from ensemble_vit import EnsembleViT
from ensemble_vit_coarse import EnsembleViTCoarse
...
group.add_argument('--num-coarse-classes', type=int, default=None, metavar='N',
                   help='number of label super classes (Model default if None)')
...
def main():
    ...
    if args.model == 'efficientnet_b0':
        model = EnsembleEfficientNetCoarse(num_classes=args.num_classes,
        ...
        model = EnsembleEfficientNetB1Coarse(num_classes=args.num_classes, num_coarse_classes=args.num_coarse_classes,
                                             cut_point=args.cut_point, width_ratio=args.width_ratio,
                                             last_channels=args.head_channels)
    elif args.model == 'resnet50':
        model = EnsembleResnet50Coarse(num_classes=args.num_classes,
                                       num_coarse_classes=args.num_coarse_classes,
                                       cut_point=args.cut_point,
                                       head_channels=args.head_channels)

    elif args.model == 'vit':
        model = EnsembleViTCoarse(
            image_size=224, num_classes=args.num_classes, num_coarse_classes=args.num_coarse_classes, cut_point=args.cut_point)
    ...
    if args.dataset == 'torch/cifar100':
        ...
        fine_to_coarse = get_fine_to_coarse_map_for_tiered_imagenet(
            args.data_dir, dataset_train.class_to_idx, args.num_classes, args.num_coarse_classes).to(device=device)
    ...
    train_loss_fn = EnsembleContrastiveLoss(
        train_ensemble_loss, eps=args.contrastive_eps, lamb=contrastive_weight).to(device=device)
    validate_loss_fn = EnsembleContrastiveLoss(
        valid_ensemble_loss, eps=args.contrastive_eps, lamb=contrastive_weight).to(device=device)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            acc1 = utils.accuracy(output[0], super_target, topk=(1,))[0]
            acc2 = utils.accuracy(output[1], super_target, topk=(1,))[0]
    ...
    mlflow.log_metric("Valid NN-12 accuracy", acc_comb_m.avg, step=epoch)

    return metrics


def get_fine_to_coarse_map_for_tiered_imagenet(root, class_to_idx, num_classes, num_coarse_classes):
    df = pd.read_csv(os.path.join(root, "combined.csv"), header=None)
    mapping = dict(zip(df.iloc[:, 0], df.iloc[:, 1]))
    fine_to_coarse = torch.zeros(num_classes, dtype=torch.long)

    # get unique coarse classes
    coarse_classes = df.iloc[:, 1].unique()
    coarse_class_to_idx = {coarse_class: i for i,
                           coarse_class in enumerate(coarse_classes)}

    for i, (fine_class, coarse_class) in enumerate(mapping.items()):
        fine_to_coarse[class_to_idx[fine_class]
                       ] = coarse_class_to_idx[coarse_class]
