...
"""
# TODO: Incomplete logging and validate (eval) function
import argparse
import logging
import os
import time
...
class EnsembleLoss(object):
    ...
    def __call__(self, output, target):
        # Indidvidual upstream model losses
        y1_loss = self.loss_fn(output[0], target)
        y2_loss = self.loss_fn(output[1], target)
        y3_loss = self.loss_fn(output[2], target)

        # Combination of two upstream models losses
        y12_loss = self.loss_fn(output[3], target)
        y13_loss = self.loss_fn(output[4], target)
        y23_loss = self.loss_fn(output[5], target)

        # Downstream model loss
        y_comb_loss = self.loss_fn(output[6], target)

        return self.weights[0] * (y1_loss + y2_loss + y3_loss) + self.weights[1] * (y12_loss + y13_loss + y23_loss) + self.weights[2] * y_comb_loss
...
class EnsembleContrastiveLoss(object):
    def __init__(self, ensemble_loss_fn, eps=4, lamb=1e-2, use_contrastive=False):
        ...
        self.eps = eps
        self.lamb = lamb
        # TODO: Either remove or implement this correctly
    ...
    def __call__(self, output, target, output_lengths, target_lengths):
        ensemble_loss = self.ensemble_loss_fn(
            output, target, output_lengths, target_lengths)
...
group.add_argument('--train-split', metavar='NAME', default='training',
...
# --- Dataset subclass to support predefined splits

VOCAB = ['_'] + list(string.ascii_lowercase + " '")
...
def greedy_decode(log_probs, blank_id=0):
    """
    Greedy decode CTC log_probs (T, B, C) → list of strings (B,)
    """
    ...
    for indices in pred_indices:
        ...
        for i in indices:
            ...
            if i != prev and i != blank_id:
                collapsed.append(VOCAB[i])
            prev = i
        decoded.append("".join(collapsed))
    return decoded


def word_error_rate(log_probs, targets, target_lengths):
    references = []
    start = 0
    for length in target_lengths:
        label = targets[start: start + length]
        ref = "".join([VOCAB[i.item()] for i in label])
        references.append(ref)
        start += length
    hypotheses = greedy_decode(log_probs)
...
class SpeechCommandsMelDataset(Dataset):
    def __init__(self,
                 ...
                 image_size: int = 224):
        """
        root_dir: path to SpeechCommands root (with train/ validation/ test lists in txt)
        ...
        # load the official split file list
        list_fname = f"{split}_list.txt"
        list_path = Path(root_dir) / list_fname
        if not list_path.exists():
            raise FileNotFoundError(f"Cannot find split file {list_path}")

        # each line is relative path like "yes/0a7c2a8d_nohash_0.wav"
        rel_paths = [l.strip() for l in open(list_path) if l.strip()]
        self.files = [Path(root_dir) / p for p in rel_paths]
    ...
    def __getitem__(self, idx):
        ...
        # 3) mel spectrogram → (1, n_mels, time)
        ...
        # 7) remove batch dim → (1, H, W), then repeat to 3 channels → (3, H, W)
        target = torch.tensor(
            [char2idx[c] for c in label.lower() if c in char2idx], dtype=torch.long)
...
class LibriSpeechMelDataset(Dataset):
    def __init__(self,
                 ...
                 n_mels: int = 128):
        """
        root_dir: directory where LibriSpeech is/will be stored
        subset: one of "train-clean-100", "train-clean-360", "train-other-500", etc.
        """
        super().__init__()
        self.dataset = LIBRISPEECH(root=root_dir, url=split, download=True)
        self.sample_rate = sample_rate
    ...
    def __getitem__(self, idx):
        ...
        target = torch.tensor(
            [char2idx[c] for c in transcript if c in char2idx], dtype=torch.long)
...
def ctc_collate(batch):
    ...
    # Pad each mel to match max_time
    padded_mels = [
        F.pad(m, (0, max_time - m.shape[-1]))  # pad on time axis (last dim)
        for m in mels
    ]

    mels = torch.stack(padded_mels)  # shape: [batch, 1, n_mels, max_time]

    # Flatten targets and get lengths
    flat_targets = torch.cat(targets)
    target_lengths = torch.tensor([len(t) for t in targets], dtype=torch.long)

    return mels, flat_targets, target_lengths

# ---- 2. DataLoader ----


def make_dataloader(root_dir: str,
                    split: str,
                    batch_size: int = 32,
                    num_workers: int = 1,
                    shuffle: bool = True,
                    dataset: str = 'speech'):
    if dataset == 'speech':
        ds = SpeechCommandsMelDataset(root_dir, split=split)
    elif dataset == 'librispeech':
        ds = LibriSpeechMelDataset(root_dir, split=split)
...
def main():
    ...
    #                              **args.model_kwargs)
    if args.model == 'deepspeech2':
        model = EnsembleDeepSpeech2(
            n_feats=128, n_tokens=n_tokens, num_rnn_layers=args.cut_point, hidden_size=512, rnn_dropout=0.1)
    ...
    # dataset_train = SubsetSC(args.data_dir, 'training')
    # dataset_eval = SubsetSC(args.data_dir, 'validation')
    ...
    loader_train = make_dataloader(
        args.data_dir, args.train_split, batch_size=args.batch_size, dataset=args.dataset)
    loader_eval = make_dataloader(
        args.data_dir, args.val_split, batch_size=args.batch_size, dataset=args.dataset)
    ...
    train_ensemble_loss = EnsembleLoss(
        nn.CTCLoss(blank=0, zero_infinity=True), args.loss_weights)
    ...
    if utils.is_primary(args):
        ...
        decreasing = True if eval_metric in [
            'loss', 'wer_comb', 'wer1', 'wer2'] else False
    ...
    with mlflow.start_run(run_name=args.run_name):
        try:
            ...
            mlflow.log_param("exit1_weight", args.loss_weights[0])
            mlflow.log_param("exit2_weight", args.loss_weights[1])
            mlflow.log_param("exit12_weight", args.loss_weights[2])
            mlflow.log_param("contrastive weight", contrastive_weight)
            mlflow.log_param("contrastive eps", args.contrastive_eps)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, target_lengths) in enumerate(loader):
        ...
        if not args.prefetcher:
            input, target, target_lengths = input.to(
                device), target.to(device), target_lengths.to(device)
        ...
        def _forward():
            with amp_autocast():
                output = model(input)
                output_lengths = model.get_output_lengths()
                loss = loss_fn(output, target, output_lengths,
                               target_lengths, )
            if accum_steps > 1:
                loss /= accum_steps

            diff = torch.sum((output[0] - output[1])**2) / output[0].size(0)
            diff_m.update(diff)
            return loss
        ...
        mlflow.log_metric("Train loss", losses_m.avg, step=num_updates)
        mlflow.log_metric("Intermediate output diff",
                          diff_m.avg, step=num_updates)
        mlflow.log_metric("Learning rate", args.lr, step=num_updates)
        ...
        if update_idx % args.log_interval == 0:
            ...
            if utils.is_primary(args):
                _logger.info(
                    ...
                    f'Diff: {diff_m.val:#.3g} ({diff_m.avg:#.3g})'
...
def validate(
    ...
    losses_m = utils.AverageMeter()
    diff_m = utils.AverageMeter()
    wer1_m = utils.AverageMeter()
    wer2_m = utils.AverageMeter()
    wer3_m = utils.AverageMeter()

    wer12_m = utils.AverageMeter()
    wer13_m = utils.AverageMeter()
    wer23_m = utils.AverageMeter()
    wer_comb_m = utils.AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target, target_lengths) in enumerate(loader):
            ...
            # wer1, acc5 = utils.accuracy(output, target, topk=(1, 5))
            wer1 = word_error_rate(output[0], target, target_lengths)
            wer2 = word_error_rate(output[1], target, target_lengths)
            wer3 = word_error_rate(output[2], target, target_lengths)
            wer12 = word_error_rate(output[3], target, target_lengths)
            wer13 = word_error_rate(output[4], target, target_lengths)
            wer23 = word_error_rate(output[5], target, target_lengths)
            wer_comb = word_error_rate(output[6], target, target_lengths)
            ...
            wer1_m.update(wer1, output[0].size(0))
            wer2_m.update(wer2, output[1].size(0))
            wer3_m.update(wer3, output[2].size(0))
            wer12_m.update(wer12, output[3].size(0))
            wer13_m.update(wer13, output[4].size(0))
            wer23_m.update(wer23, output[5].size(0))

            wer_comb_m.update(wer_comb, output[2].size(0))
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    f'Wer@1: {wer1_m.val:>7.3f} ({wer1_m.avg:>7.3f})  '
                    f'Wer@2: {wer2_m.val:>7.3f} ({wer2_m.avg:>7.3f})'
                    f'Wer@comb: {wer_comb_m.val:>7.3f} ({wer_comb_m.avg:>7.3f})'
                )

    metrics = OrderedDict(
        [('loss', losses_m.avg), ('wer1', wer1_m.avg),
         ('wer2', wer2_m.avg), ('wer_comb', wer_comb_m.avg), ("diff", diff_m.avg)])

    mlflow.log_metric("valid loss", losses_m.avg, step=epoch)
    mlflow.log_metric("Valid Intermediate Diff", diff_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-1 word error rate", wer1_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-2 word error rate", wer2_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-12 word error rate",
                      wer_comb_m.avg, step=epoch)
