...
class EnsembleResnet50Coarse(nn.Module):
    def __init__(self, num_classes: int, num_coarse_classes: int, cut_point: int, head_channels: int = 320):
        super(EnsembleResnet50Coarse, self).__init__()
        self.num_classes = num_classes
        ...
        self.encoder1 = Resnet50Early(
            self.num_classes, num_coarse_classes, self._cut_point, use_head=False, head_channels=self._head_channels)
        ...
        print("Ensemble Resnet50Coarse created")
        print(
            f"Encoder-1 # params: {self.encoder1_stat.total_params}, last channels {self.encoder1.head_channels}")
        print(
            f"Encoder-2 # params: {self.encoder2_stat.total_params}, last channels {self.encoder2.head_channels}")
...
class Resnet50Early(nn.Module):
    def __init__(self, num_classes: int, num_coarse_classes: int, cut_point: int, use_head: bool = False, head_channels: int = 1280):
        ...
        print('Warning: Currently not making use of use_head in Resnet50Early')
        self.classifier = LinearClassifier(
            self.num_coarse_classes, self.encoder.head_channels)

        # if use_head:
        #     self.classifier = ResNetHead(
        #         self.num_coarse_classes, self.encoder.head_channels, self.head_channels)
        # else:
        #     self.classifier = LinearClassifier(
        #         self.num_coarse_classes, self.encoder.head_channels)
