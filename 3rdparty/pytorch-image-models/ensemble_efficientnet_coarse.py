...
class EfficientNetHead(nn.Module):
    ...
    def forward(self, inputs_1, inputs_2):
        x1 = self._classifier_in(inputs_1)
        x2 = self._classifier_in(inputs_2)
...
#         self._conv_head = get_same_padding_conv2d(input_filters, self.out_channels, kernel_size=1)
...
class EnsembleEfficientNetCoarse(nn.Module):
    def __init__(self, num_classes: int, num_coarse_classes: int, cut_point: int, width_ratio: float = 1.0, num_repeat: int = None,
        ...
        super(EnsembleEfficientNetCoarse, self).__init__()
        ...
        self.encoder1 = EfficientNetEarly(
            num_classes=num_classes, num_coarse_classes=num_coarse_classes, cut_point=cut_point)
    ...
    def forward(self, inputs):
        ...
        x1 = self.encoder1.encoder(inputs)
        x2 = self.encoder2.encoder(inputs)

        y1 = self.encoder1(inputs)
        y2 = self.encoder2(inputs)

        y_comb = self.classifier_comb(x1, x2)
    ...
    def initialize_encoders(self, nn1_checkpoint_path: str = '', nn2_checkpoint_path: str = '', device: str = 'cpu'):
        ...
        if nn2_checkpoint_path:
            checkpt_nn2 = torch.load(nn1_checkpoint_path, map_location=device)
    ...
    def freeze_and_unfreeze_encoders(self, freeze_nn1: bool = False, freeze_nn2: bool = False):
        # Freeze NN-1 and NN-2 encoder weights
        if freeze_nn1:
            print("Freezing NN-1")
