...
from dataset_utils.data_utils import partition_data_on_coarse_labels
...
class EnsembleLoss(object):
    ...
    def __call__(self, output, target, super_target, second_half):
        y_comb_loss = self.weights[2] * self.loss_fn(output[2], target)

        if not second_half:
            y_comb_loss += self.weights[0] * \
                self.loss_fn(output[0], super_target)
        else:
            y_comb_loss += self.weights[1] * \
                self.loss_fn(output[1], super_target)
...
class EnsembleContrastiveLoss(object):
    ...
    def __call__(self, output, target, super_target, second_half):
        ensemble_loss = self.ensemble_loss_fn(
            output, target, super_target, second_half)
...
group.add_argument('--second-half', default=False, type=bool, dest="second_half",
                   help='Second part of training data  (default: False)')
...
def main():
    ...
    if args.model == 'efficientnet_b0':
        ...
        model = EnsembleResnet50Coarse(num_classes=args.num_classes, num_coarse_classes=args.num_coarse_classes,
    ...
    print(f'Second half partition for data : {args.second_half}')
    if args.second_half:
        partition = set(range(10, 20))
    else:
        partition = set(range(10))

    dataset_train = partition_data_on_coarse_labels(args.data_dir, partition)
    ...
    dataset_eval = partition_data_on_coarse_labels(
        args.data_dir, partition, train=False)
    ...
    validate_loss_fn = EnsembleContrastiveLoss(valid_ensemble_loss, eps=args.contrastive_eps,
                                               lamb=contrastive_weight,  use_contrastive=args.use_contrastive).to(device=device)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                output = model(input)
                loss = loss_fn(output, target, super_target, second_half)
            if accum_steps > 1:
                loss /= accum_steps
