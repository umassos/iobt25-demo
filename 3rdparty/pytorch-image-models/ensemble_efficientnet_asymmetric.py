...
class EfficientNetHeadAsymmetric(nn.Module):
    def __init__(self, num_classes: int, input_filters_1: int, input_filters_2: int, out_channels: int = 1280, drop_out_rate: float = 0.3):
        super(EfficientNetHeadAsymmetric, self).__init__()
        ...
        self._linear = nn.Linear(
            self.input_filters_1 + self.input_filters_2, self._num_classes)
...
class EnsembleEfficientNetAsymmetric(nn.Module):
    def __init__(self, num_classes: int, cut_point_1: int, cut_point_2: int, width_ratio: float = 1.0, num_repeat: int = None,
                 last_channels: int = 1280):
        super(EnsembleEfficientNetAsymmetric, self).__init__()
        self.num_classes = num_classes
        self.num_repeat = num_repeat  # Number of repeat for the final block
        self.last_channels = last_channels
        self._cut_point_1 = cut_point_1
        self._cut_point_2 = cut_point_2
        self.width_ratio = width_ratio

        self.encoder1 = EfficientNetEarly(
            num_classes=num_classes, cut_point=cut_point_1, last_channels=last_channels)
        self.encoder2 = EfficientNetEarly(
            num_classes=num_classes, cut_point=cut_point_2, last_channels=last_channels)
        ...
        self.classifier_comb = EfficientNetHeadAsymmetric(
            self.num_classes, self.encoder1.encoder.last_channels, self.encoder2.encoder.last_channels, self.last_channels)
...
class EfficientNetEarly(nn.Module):
    def __init__(self, num_classes: int, cut_point: int, use_head: bool = False, last_channels: int = 1280):
        ...
        if use_head:
            self.classifier = EfficientNetHeadAsymmetric(
