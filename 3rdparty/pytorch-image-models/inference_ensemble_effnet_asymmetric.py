...
from ensemble_efficientnet import EnsembleEfficientNet
...
group.add_argument('--run-name', default='default', type=str, metavar='NAME', help='Name of this run')
...
def main():
    ...
    test_ensemble_loss = EnsembleLoss(torch.nn.CrossEntropyLoss(), args.loss_weights)
    ...
    test_loss_fn = EnsembleContrastiveLoss(test_ensemble_loss, eps=args.contrastive_eps, lamb=contrastive_weight).to(device=device)
    with mlflow.start_run(run_name=args.run_name):
        mlflow.log_param("batch_size", args.batch_size)
        mlflow.log_param("NN-1 encoder number of parameters", model.encoder1_stat.total_params)
        mlflow.log_param("NN-1 classifier number of parameters", model.classifier1_stat.total_params)
        mlflow.log_param("NN-2 encoder number of parameters", model.encoder2_stat.total_params)
        mlflow.log_param("NN-2 classifier number of parameters", model.classifier2_stat.total_params)
        mlflow.log_param("NN-12 number of parameters", model.classifier_comb_stat.total_params)
        eval_metrics = test(model, loader, test_loss_fn, args, fine_to_coarse, amp_autocast=amp_autocast, device=device)
...
def class_wise_accuracy(output, target, num_classes=100, topk=(1,)):
    ...
    class_correct = torch.zeros(num_classes, dtype=torch.float, device=output.device)
    class_total = torch.zeros(num_classes, dtype=torch.float, device=output.device)
    ...
    class_accuracy = torch.where(class_total > 0, class_correct / class_total * 100, torch.tensor(0.0, device=output.device))

    return class_accuracy, class_total  # Returns a tensor of shape (num_classes,) with accuracy per class
...
def test(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            acc1 = super_accuracy(output[0], target, fine_to_coarse, device, topk=(1,))[0]
            acc2 = super_accuracy(output[1], target, fine_to_coarse, device, topk=(1,))[0]
            acc_comb = super_accuracy(output[2], target, fine_to_coarse, device, topk=(1,))[0]
