...
from ensemble_deepspeech2 import EnsembleDeepSpeech2, DeepSpeech2Early
...
group.add_argument("--cut-point", type=int, default=0, dest="cut_point",
...
def main():
    ...
    cut_point = args.cut_point if args.cut_point > 0 else 5
    if args.model == 'deepspeech2':
        model = DeepSpeech2Early(
            n_feats=128, n_tokens=n_tokens, num_rnn_layers=cut_point, hidden_size=512, rnn_dropout=0.1)
    ...
    train_loss_fn = nn.CTCLoss(blank=0, zero_infinity=True,).to(device)
    validate_loss_fn = nn.CTCLoss(blank=0, zero_infinity=True).to(device)
...
def train_one_epoch(
    ...
    for batch_idx, (input, target, target_lengths) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                output, output_lengths = model(input)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target, target_lengths) in enumerate(loader):
            ...
            wer = word_error_rate(output, target, target_lengths)
            ...
            wer_m.update(wer, output[0].size(0))
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    f'Wer: {wer_m.val:>7.3f} ({wer_m.avg:>7.3f})  '
                )

    metrics = OrderedDict(
        [('loss', losses_m.avg), ('wer', wer_m.avg)])

    mlflow.log_metric("valid loss", losses_m.avg, step=epoch)
    mlflow.log_metric("Valid word error rate", wer_m.avg, step=epoch)
