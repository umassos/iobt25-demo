...
class EnsembleEfficientNetB1Coarse(nn.Module):
    def __init__(self, num_classes: int, num_coarse_classes: int, cut_point: int, width_ratio: float = 1.0, num_repeat: int = None,
                 last_channels: int = 1280):
        super(EnsembleEfficientNetB1Coarse, self).__init__()
        ...
        # self.classifier1 = LinearClassifier(self.num_classes, self.encoder1.last_channels)
        # self.classifier2 = LinearClassifier(self.num_classes, self.encoder2.last_channels)
        self.encoder1 = EfficientNetEarly(
            num_classes=num_classes, cut_point=cut_point, num_coarse_classes=num_coarse_classes)
        self.encoder2 = EfficientNetEarly(
            num_classes=num_classes, cut_point=cut_point, num_coarse_classes=num_coarse_classes)

        self.classifier_comb = EfficientNetHead(self.num_classes,
                                                self.encoder1.encoder.last_channels + self.encoder2.encoder.last_channels)
        ...
        print("Ensemble EfficientNet B1 created")
        print(
...
class EfficientNetEarly(nn.Module):
    def __init__(self, num_classes: int, num_coarse_classes: int, cut_point: int, use_head: bool = False, last_channels: int = 1280):
        ...
        self.encoder = get_multiexit_efficientnet_b1(
            self.num_classes, 0, self._cut_point, 1, 1)

        if use_head:
            self.classifier = EfficientNetHead(
                self.num_coarse_classes, self.encoder.last_channels, self.last_channels)
        else:
            self.classifier = LinearClassifier(
                self.num_coarse_classes, self.encoder.last_channels)
