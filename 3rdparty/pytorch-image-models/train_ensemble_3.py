...
from ensemble_efficient_net_b0_3 import EnsembleEfficientNet
from ensemble_resnet_50_3 import EnsembleResnet50
from ensemble_vit_3 import EnsembleViT
...
group.add_argument("--freeze-nn3", type=bool, default=False, dest="freeze_nn3",
                   help="Freeze NN-3 weights while training")
...
group.add_argument('--checkpoint-nn3', default='', type=str, metavar='PATH', dest="checkpt_nn3",
                   help='Initialize NN-3 Backbone model from this checkpoint (default: none)')
...
def main():
    ...
    if args.model == 'efficientnet_b0':
        ...
        model = EnsembleResnet50(num_classes=args.num_classes,
                                 cut_point=args.cut_point, head_channels=args.head_channels)
    elif args.model == 'vit':
        model = EnsembleViT(
    ...
    if args.checkpt_nn1 or args.checkpt_nn2 or args.checkpt_nn3:
        model.initialize_encoders(
            args.checkpt_nn1, args.checkpt_nn2, args.checkpt_nn3, device)

    if args.freeze_nn1 or args.freeze_nn2 or args.freeze_nn3:
        model.freeze_and_unfreeze_encoders(
            args.freeze_nn1, args.freeze_nn2, args.freeze_nn3)
    ...
    with mlflow.start_run(run_name=args.run_name):
        try:
            ...
            mlflow.log_param("exit12_weight", args.loss_weights[1])
            mlflow.log_param("exit123_weight", args.loss_weights[2])
            ...
            mlflow.log_param("NN-1 encoder number of parameters",
                             raw_model._encoder1_params)
            mlflow.log_param("NN-2 encoder number of parameters",
                             raw_model._encoder2_params)
            mlflow.log_param("NN-3 encoder number of parameters",
                             raw_model._encoder3_params)

            mlflow.log_param("NN-1 classifier number of parameters",
                             raw_model._classifier1_params)
            mlflow.log_param("NN-2 classifier number of parameters",
                             raw_model._classifier2_params)
            mlflow.log_param("NN-3 classifier number of parameters",
                             raw_model._classifier3_params)

            mlflow.log_param("NN-12 number of parameters",
                             raw_model._classifier12_params)
            mlflow.log_param("NN-23 number of parameters",
                             raw_model._classifier23_params)
            mlflow.log_param("NN-13 number of parameters",
                             raw_model._classifier13_params)

            mlflow.log_param("NN-123 number of parameters",
                             raw_model._classifier_comb_params)
...
def validate(
    ...
    acc3_m = utils.AverageMeter()

    acc12_m = utils.AverageMeter()
    acc13_m = utils.AverageMeter()
    acc23_m = utils.AverageMeter()
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            acc3 = utils.accuracy(output[2], target, topk=(1,))[0]
            acc12 = utils.accuracy(output[3], target, topk=(1,))[0]
            acc13 = utils.accuracy(output[4], target, topk=(1,))[0]
            acc23 = utils.accuracy(output[5], target, topk=(1,))[0]
            acc_comb = utils.accuracy(output[6], target, topk=(1,))[0]
            ...
            acc3_m.update(acc3.item(), output[2].size(0))
            acc12_m.update(acc12.item(), output[3].size(0))
            acc13_m.update(acc13.item(), output[4].size(0))
            acc23_m.update(acc23.item(), output[5].size(0))
            acc_comb_m.update(acc_comb.item(), output[6].size(0))
            ...
            if utils.is_primary(args) and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    f'Acc@3: {acc3_m.val:>7.3f} ({acc3_m.avg:>7.3f})'
                    f'Acc@12: {acc12_m.val:>7.3f} ({acc12_m.avg:>7.3f})'
                    f'Acc@13: {acc13_m.val:>7.3f} ({acc13_m.avg:>7.3f})'
                    f'Acc@23: {acc23_m.val:>7.3f} ({acc23_m.avg:>7.3f})'
    ...
    metrics = OrderedDict(
        [('loss', losses_m.avg),
         ...
         ('acc13', acc13_m.avg),
         ('acc23', acc23_m.avg),
         ('acc_comb', acc_comb_m.avg),
         ('diff', diff_m.avg)])

    mlflow.log_metric("valid loss", losses_m.avg, step=epoch)
    mlflow.log_metric("Valid Intermediate Diff", diff_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-1 accuracy", acc1_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-2 accuracy", acc2_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-3 accuracy", acc3_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-13 accuracy", acc13_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-12 accuracy", acc12_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-23 accuracy", acc23_m.avg, step=epoch)
    mlflow.log_metric("Valid NN-123 accuracy", acc_comb_m.avg, step=epoch)
