...
**S1 node:**
```bash
MODEL=ensemble-effnet-c5-lr-0.005-tin HEAD=192.168.82.12:8185 docker compose -f docker-compose.s1.yml up --build
```

**S2 node:**
```bash
MODEL=ensemble-effnet-c5-lr-0.005-tin HEAD=192.168.82.12:8185 docker compose -f docker-compose.s2.yml up --build
```
...
**Original only:**
```bash
python3 system/metrics_client.py -d 150 -w -i effnet_baseline_original
```

**MEL ensemble only (kill original first, then restart and skip to MEL):**
```bash
python3 system/metrics_client.py -d 150 -f -w -i effnet_baseline_mel
```

**S1 solo only (kill original + S2, then restart and skip to S1):**
```bash
python3 system/metrics_client.py -d 150 -f -w -i effnet_baseline_s1
```
...
```bash
python3 system/metrics_client.py -d 150 -f -w -i effnet_fail_original
```

In a separate terminal, inject the failure after a delay (e.g. 10 seconds):
```bash
./fail_original.sh 10 10 effnet_fail_original effnet-b0
```
...
Start the client with failover enabled:
```bash
python3 system/metrics_client.py -d 150 -f -w -i effnet_fail_s2
```

In a separate terminal, inject the failure after a delay (e.g. 10 seconds):
```bash
./fail_s2.sh 10 10 effnet_fail_s2 ensemble-effnet-c5-lr-0.005-tin 192.168.82.12:8185
```
...
**S1 node:**
```bash
MODEL=ensemble-deepspeech2-c2-librispeech HEAD=192.168.82.12:8185 docker compose -f docker-compose.s1.yml up --build
```

**S2 node:**
```bash
MODEL=ensemble-deepspeech2-c2-librispeech HEAD=192.168.82.12:8185 docker compose -f docker-compose.s2.yml up --build
```
...
**Original only:**
```bash
python3 system/metrics_client.py -d 150 -w -i deepspeech_baseline_original --task deepspeech
```

**MEL ensemble only:**
```bash
python3 system/metrics_client.py -d 150 -f -w -i deepspeech_baseline_mel --task deepspeech
```

**S1 solo only:**
```bash
python3 system/metrics_client.py -d 150 -f -w -i deepspeech_baseline_s1 --task deepspeech
```

### 3. Experiment A — Failure of original server

```bash
python3 system/metrics_client.py -d 150 -f -w -i deepspeech_fail_original --task deepspeech
```

In a separate terminal:
```bash
./fail_original.sh 10 10 deepspeech_fail_original deepspeech2
```
...
```bash
python3 system/metrics_client.py -d 150 -f -w -i deepspeech_fail_s2 --task deepspeech
```

In a separate terminal:
```bash
./fail_s2.sh 10 10 deepspeech_fail_s2 ensemble-deepspeech2-c2-librispeech 192.168.82.12:8185
```
...
```bash
python3 system/metrics_client.py -d 150 -f -w -i colocated_s1_fail_original --config system/config.py
python3 system/metrics_client.py -d 150 -f -w -i colocated_s1_ds_fail_original --task deepspeech --config system/config2.py
```

In a separate terminal:
```bash
./fail_original.sh 10 10 colocated_s1_fail_original effnet-b0
```
...
Run both clients simultaneously:
```bash
python3 system/metrics_client.py -d 150 -f -w -i colocated_s1_fail_s2 --config system/config.py
python3 system/metrics_client.py -d 150 -f -w -i colocated_s1_ds_fail_s2 --task deepspeech --config system/config2.py
```

In a separate terminal:
```bash
./fail_s2.sh 10 10 colocated_s1_fail_s2 ensemble-effnet-c5-lr-0.005-tin 192.168.82.12:8185
```
...
Run both clients simultaneously:
```bash
python3 system/metrics_client.py -d 150 -f -w -i colocated_head_fail_original --config system/config.py
python3 system/metrics_client.py -d 150 -f -w -i colocated_head_ds_fail_original --task deepspeech --config system/config2.py
```

In a separate terminal:
```bash
./fail_original.sh 10 10 colocated_head_fail_original effnet-b0
```
